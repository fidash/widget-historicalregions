/*global MashupPlatform, AmCharts, $, d3 */
...
/* exported WidgetHistoricalregions */

var WidgetHistoricalregions = (function () {
    ...
    var WidgetHistoricalregions = function WidgetHistoricalregions () {
        this.token = "";
        this.chart = null;
        this.fulldata = {};
        this.limits = true;
    ...
    var drawHorizon = function drawHorizon(fullData, region, width) {
        ...
        $("<div></div>", {class: "regionTitle", text: region})
            .appendTo($("#" + region));

        $("<div></div>", {class: "dataText", text: "RAM"}).appendTo($("#" + region));
        d3.select("#" + region)
            .append("svg")
        ...
        $("<div></div>", {class: "dataText", text: "VCPU"}).appendTo($("#" + region));
        d3.select("#" + region)
            .append("svg")
            .attr("class", "horizonGraph")
            .attr("width", width)
        ...
        $("<div></div>", {class: "dataText", text: "DISK"}).appendTo($("#" + region));
        d3.select("#" + region)
            .append("svg")
            .attr("class", "horizonGraph")
            .attr("width", width)
    ...
    var viewRegion = function viewRegion() {
        ...
        var fullData = this.fulldata[this.singleregion];
        if (!fullData) {
            return;
        }
        ...
        this.chart = viewSingleRegion.call(this, fullData, this.singleregion, multi, limit);
    };
        ...
        var resizeHorizonDebounced = debouncer(function () {
            ...
            for (var region in regions) {
                if (regions.hasOwnProperty(region)) {
                    // clear
                    $("#" + region).remove();
                    drawHorizon.call($this, regions[region], region, width);
                }
            }
        });
    ...
    return WidgetHistoricalregions;
